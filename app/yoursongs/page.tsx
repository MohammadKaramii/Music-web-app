...
const UserSongs = () => {
  ...
  const { data: userSongs = [], isLoading, error, refetch } = useSongsByUser(user?.id || "");
  ...
  return (
    <div className="bg-neutral-900 rounded-lg h-full w-full overflow-hidden overflow-y-auto">
        ...
        <div className="mt-20">
          <div className="flex flex-col items-center md:flex-row gap-x-5">
            <div className="flex flex-col gap-y-2 mt-4 md:mt-0">
              <h1 className="text-6xl text-white sm:text-5xl lg:text-4xl font-bold">Your Added Songs</h1>
              {userSongs.length > 0 && (
      ...
      {user ? (
        error ? (
          <div className="px-6">
            <ErrorState message="Failed to load your songs. Please try again." onRetry={refetch} />
          </div>
        ) : isLoading ? (
          <ListSkeleton count={8} />
        ) : userSongs.length === 0 ? (
          <div className="px-6">
            <EmptyState title="No songs uploaded yet" description="Upload your first song to see it here!" />
          </div>
        ) : (
